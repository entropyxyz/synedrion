...
fn bench_aff_g(c: &mut Criterion) {
    ...
    group.bench_function("prove (option 2)", |b| {
        b.iter_batched(
            || aff_g_iter_batched(&mut OsRng, &KEY0, &KEY1, Measure::Creation),
            |f| f(&mut OsRng),
            BatchSize::LargeInput,
        )
    });

    group.bench_function("verify (option 2)", |b| {
        b.iter_batched(
            || aff_g_iter_batched(&mut OsRng, &KEY0, &KEY1, Measure::Verification),
            |f| f(&mut OsRng),
            BatchSize::LargeInput,
        )
    });
