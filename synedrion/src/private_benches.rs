...
where
    R: CryptoRngCore,
{
    let sk0 = key0.sk.clone();
    let pk0 = sk0.public_key().clone();

    let sk1 = key1.sk.clone();
    let pk1 = sk1.public_key().clone();

    let rp_params = key0.rp_params.clone();
    ...
    match measure {
        Measure::Creation => Box::new(move |rng| {
            ...
            AffGProof::<Params>::new(rng, secret, public, &rp_params, &aux);
        }),
...
pub fn measure_sch(rng: &mut impl CryptoRngCore, iters: u64, measure: Measure) -> Duration {
    ...
    for _ in 0..iters {
        ...
        let secret = Secret::init_with(|| Scalar::random(rng));
