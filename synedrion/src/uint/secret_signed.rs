    ...
    T: ConditionallySelectable + Zeroize + Integer + Bounded,
{
    ...
    pub fn new_modulo(value: Secret<T>, modulus: &NonZero<T>, modulus_bound: u32) -> Option<Self> {
        ...
        // can have varying size (e.g. for two random 1024 bit primes it can be 2022-2024 bits long).

        let positive = value;
        let half_modulus = modulus.as_ref().wrapping_shr_vartime(1);
        let is_negative = positive.expose_secret().ct_gt(&half_modulus);
        // Can't define a `Sub<Secret>` for `Uint`, so have to re-wrap manually.
        let negative = Secret::init_with(|| *modulus.as_ref() - positive.expose_secret()).wrapping_neg();
        let value = Secret::<T>::conditional_select(&positive, &negative, is_negative);
