...
- ECDSA Pre-Signing - performs all the signing calculations that do not depend on the message that is being signed;
- ECDSA Signing - finishes up signing given a pre-hashed message.
- ECDSA Interactive Signing - two protocols above chained one after the other acting as a single protocol.
- Threshold Key Resharing - technically not a part of the CGGMP'24 proper, but needed to enable threshold functionality.

All the protocols support identifiable aborts where specified by the paper, and where possible, a self-contained malicious behavior evidence will be returned, so that it could be published.
