use crypto_bigint::{NonZero, RandomMod, U128};
...
pub fn new_positive_with_constant_bound(runner: &mut CtRunner, rng: &mut BenchRng) {
    let bound = 127;
    let modulus = NonZero::new(U128::ONE << bound).unwrap();
    ...
    let mut inputs = {
        (0..input_len)
            .map(|_| {
                ...
                if rng.r#gen::<bool>() {
                    (Class::Left, Secret::init_with(|| U128::random_mod(rng, &modulus)))
                // "Special" class: small value.
                } else {
                    (Class::Right, Secret::init_with(|| U128::from_u128(123u128)))
                }
            })
            .collect::<Vec<(Class, Secret<U128>)>>()
    };
